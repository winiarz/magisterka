...
__kernel void nbody_withSharedMem5( __global float* g_X,  __global float* g_Y,  __global float* g_Z,
                                    ...
                                    __global float* g_m )
{
    ...
    for( uint r_k=0; r_k<c_N4/(LOCAL_SIZE*r_groupSize); r_k++)
    {
        ...
        for( uint r_j=0; r_j<c_N; r_j++)
        {
            ...
            {
            //r_rX[0] += rsqrt(l_X[get_local_id(0)] - r_x);
            //r_rY[0] += rsqrt(l_Y[get_local_id(0)] - r_y);
            //r_rZ[0] += rsqrt(l_Z[get_local_id(0)] - r_z);
            ...
            r_rX[1] += rsqrt(l_X[get_local_size(0)+get_local_id(0)] - (r_x));
            r_rY[1] += rsqrt(l_Y[get_local_size(0)+get_local_id(0)] - (r_y));
            r_rZ[1] += rsqrt(l_Z[get_local_size(0)+get_local_id(0)] - (r_z));

            r_rX[2] += rsqrt(l_X[2*get_local_size(0)+get_local_id(0)] - (r_x));
            r_rY[2] += rsqrt(l_Y[2*get_local_size(0)+get_local_id(0)] - (r_y));
            r_rZ[2] += rsqrt(l_Z[2*get_local_size(0)+get_local_id(0)] - (r_z));

            r_rX[3] += rsqrt(l_X[3*get_local_size(0)+get_local_id(0)] - (r_x));
            r_rY[3] += rsqrt(l_Y[3*get_local_size(0)+get_local_id(0)] - (r_y));
            r_rZ[3] += rsqrt(l_Z[3*get_local_size(0)+get_local_id(0)] - (r_z));
            ...
            /*{
                ...
                float4 r_dist_sq = r_dx*r_dx + r_dy*r_dy + r_dz*r_dz + c_Epsilon;
                float4 r_dist_minus1 = rsqrt( r_dist_sq);
                float4 r_dist_minus3 = r_m * r_dist_minus1 * r_dist_minus1 * r_dist_minus1;
                //float4 r_dist_minus3 = g_m[r_j] * r_dist_minus1 * r_dist_minus1 * r_dist_minus1;
            ...
            /*{
                ...
                float4 r_dist_sq = r_dx*r_dx + r_dy*r_dy + r_dz*r_dz + c_Epsilon;
                float4 r_dist_minus1 = 1.3 * r_dist_sq;//rsqrt( r_dist_sq);
                float4 r_dist_minus3 = r_m * r_dist_minus1 * r_dist_minus1 * r_dist_minus1;
                //float4 r_dist_minus3 = g_m[r_j] * r_dist_minus1 * r_dist_minus1 * r_dist_minus1;
